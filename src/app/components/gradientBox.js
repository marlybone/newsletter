...
export const TagGridItem = ({category, className, name}) => {
    ...
    <div className={`bg-gradient-to-r absolute -inset-[0.10rem] rounded-lg blur-[4px] transition-all opacity-25 duration-500 group-hover:opacity-100 group-hover:duration-200 ${className}`}   />
    <div className='relative px-3 py-2 bg-white ring-3 ring-gray-900/5 rounded-lg leading-none flex items-top justify-start space-x-6'><h2>{category}</h2>
    <h3>{name}</h3>
    </div>
    </div>
    )
